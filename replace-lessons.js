...
const oldText = "Geography Lessons+";
...
function processFile(filePath) {
  try {
    ...
    if (content.includes("<title>")) {
      const titleRegex = /<title>(.*?)Geography Lessons\+(.*?)<\/title>/g;
      const newContent = content.replace(titleRegex, "<title>$1Geography Lessons$2</title>");
